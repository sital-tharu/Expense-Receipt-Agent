...
- **Gmail intake**: dependency-free OAuth + REST (`src/lib/gmail-auth.ts`, `src/lib/gmail.ts`). Read-only scope; reads the `receipts` label (`GMAIL_LABEL`); refresh token at gitignored `secrets/gmail-token.json`; dedup via `processedEmails/{messageId}`. Image attachments reuse the multimodal path (+ stored image); HTML/text bodies go through `extractReceiptFromText`. Sync is triggered by the dashboard button → `POST /api/gmail/sync`.
